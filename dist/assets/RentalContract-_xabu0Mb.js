...
<p>3.1. Арендная плата составляет ${O(w)} рублей (${M(w)}) за ${k} за единицу Имущества.</p>
<p>3.2. Расчётный период аренды составляет ${D} ${a.rate_period==="day"?"сут.":a.rate_period==="week"?"нед.":"мес."}, общая сумма арендной платы — ${O(C)} рублей (${M(C)}).</p>
...
`.trim()}const N="w-full bg-[#161616] border border-white/10 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#D4AF37]/50",_e="flex items-center justify-center gap-2 bg-[#D4AF37] hover:bg-[#B8860B] transition-colors text-[#161616] text-sm px-4 min-h-[44px] rounded-lg disabled:opacity-40",q="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 transition-colors text-sm px-4 min-h-[44px] rounded-lg";function fe(){const{rentalId:e}=se(),a=ne(),l=m.useRef(null),[s,_]=m.useState(!0),[h,u]=m.useState(!1),[k,D]=m.useState(!1),[C,w]=m.useState(""),[L,j]=m.useState(""),[o,z]=m.useState(null),[y,B]=m.useState(null),[I,P]=m.useState(null),[g,S]=m.useState(null),[v,T]=m.useState(""),[A,W]=m.useState(!1),[p,b]=m.useState({city:"",contract_number:"",contract_date:new Date().toISOString().slice(0,10),purpose:"",delivery:"self_pickup",penalty_pct:"0,5",claim_days:"10",copies_total:"двух",extra_terms:""});m.useEffect(()=>{e&&(_(!0),Promise.all([R.list(),ae.get()]).then(async([r,i])=>{const c=(r.rentals||[]).find(x=>x.id===Number(e));if(!c){a("/cabinet/rentals");return}z(c),P(i);const $=(r.counterparties||[]).find(x=>x.id===c.counterparty_id)||null;if(B($),c.contract_id){const x=await R.contracts.get(c.contract_id);S(x),T(x.content_html),b(te=>({...te,...x.options,contract_number:x.contract_number,contract_date:String(x.contract_date).slice(0,10)}))}else b(x=>({...x,contract_number:c.rental_number}))}).catch(r=>w(r instanceof Error?r.message:"Не удалось загрузить")).finally(()=>_(!1)))},[e]);const H=m.useMemo(()=>o?me({rental:o,counterparty:y,company:I,options:p}):"",[o,y,I,p]);m.useEffect(()=>{!o||A||g&&v||T(H)},[H,o,A,g,v]);const G=m.useMemo(()=>y?le(y):["контрагент не выбран"],[y]),X=()=>{T(H),W(!1),l.current&&(l.current.innerHTML=H),j("Текст пересобран по данным аренды"),setTimeout(()=>j(""),2500)},Y=async()=>{var r;if(o){u(!0),w("");try{const i=((r=l.current)==null?void 0:r.innerHTML)||v,c={rental_id:o.id,contract_number:p.contract_number||o.rental_number,contract_date:p.contract_date||new Date().toISOString().slice(0,10),status:(g==null?void 0:g.status)||"draft",options:p,content_html:i,total_amount:U(o)};if(g)await R.contracts.update(g.id,c);else{const $=await R.contracts.create(c);S({id:$.id,rental_id:o.id,contract_number:c.contract_number,contract_date:c.contract_date,status:"draft",options:c.options,content_html:i,total_amount:c.total_amount,created_at:new Date().toISOString()})}T(i),j("Договор сохранён"),setTimeout(()=>j(""),2500)}catch(i){w(i instanceof Error?i.message:"Не удалось сохранить")}finally{u(!1)}}},Z=()=>{var $;const r=(($=l.current)==null?void 0:$.innerHTML)||v,i=`Договор аренды № ${p.contract_number||(o==null?void 0:o.rental_number)||""}`,c=window.open("","_blank");c&&(c.document.write(`
      <html>
        <head>
          <title>${i}</title>
          <style>
