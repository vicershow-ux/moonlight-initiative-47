const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-BIYreVBH.js","assets/index-Dzh3pBz5.js","assets/index-CXOFTxKe.css"])))=>i.map(i=>d[i]);
import{_ as $}from"./index-Dzh3pBz5.js";const v=t=>String(t??"").replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\r?\n/g," "),b=(t,r=0)=>{const e=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(e)?e:r},g=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(b(t))+" ₽",A=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function l(t){const r=document.createElement("div");return r.textContent=t??"",r.innerHTML}function _(t,r){const e=new Map;return t.forEach(n=>{const a=r.find(i=>i.id===n.material_id),d=n.shop_name||(a==null?void 0:a.shop_name)||"Магазин не указан";e.has(d)||e.set(d,{name:d,address:"",phone:"",items:[],sum:0});const o=e.get(d);!o.address&&(a!=null&&a.shop_address)&&(o.address=a.shop_address),!o.phone&&(a!=null&&a.shop_phone)&&(o.phone=a.shop_phone),o.items.push(n),o.sum+=b(n.qty)*b(n.price)}),Array.from(e.values()).sort((n,a)=>a.sum-n.sum)}function y(t,r,e,n){const a=_(r,e),d=a.reduce((c,u)=>c+u.sum,0),o=Array.from(new Set(r.map(c=>c.room_name).filter(Boolean))),i=o.length?o.join(", "):"Без помещения",s=a.map(c=>{const u=c.items.map((h,k)=>`
            <tr>
              <td class="num">${k+1}</td>
              <td>${l(h.name)}</td>
              <td>${l(h.room_name||"—")}</td>
...
`,f=`
  ...
  </div>`,m=`Смета на материал № ${t.object_code} — ${l(t.client_name)}`;return{styles:p,bodyContent:f,title:m}}function E(t,r,e,n,a=!1){const{styles:d,bodyContent:o,title:i}=y(t,r,e,n),s=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${i}</title>
...
</html>`,p=window.open("","_blank","width=900,height=1000");p&&(p.document.open(),p.document.write(s),p.document.close(),a&&(p.onload=()=>{p.focus(),p.print()}))}function w(t,r){let e=t;for(;;){const n=r.exec(e);if(!n)return e;const a=e.indexOf("{",n.index);if(a===-1)return e;let d=0,o=-1;for(let i=a;i<e.length;i++)if(e[i]==="{")d++;else if(e[i]==="}"&&(d--,d===0)){o=i;break}if(o===-1)return e;e=e.slice(0,n.index)+e.slice(o+1),r.lastIndex=0}}function z(t,r){return w(w(t,/@page\b/g),/@media\s+print\b/g).replace(/(^|\})\s*([^{}@]+)\s*\{/g,(n,a,d)=>{const o=d.split(",").map(i=>{const s=i.trim();return s?s==="*"?`${r}, ${r} *`:/^(html|body)$/i.test(s)||s===".est-root"?r:s.startsWith(".est-root ")?`${r} ${s.slice(10)}`:`${r} ${s}`:""}).filter(Boolean).join(", ");return`${a} ${o} {`})}async function j(t,r,e,n){const{styles:a,bodyContent:d}=y(t,r,e,n),o=document.createElement("div");o.style.position="fixed",o.style.left="-10000px",o.style.top="0",o.style.width="760px",o.style.background="#ffffff";const i=document.createElement("div");i.id="pdf-scope-materials",i.className="est-root",i.style.width="760px",i.style.maxWidth="760px",i.style.padding="0",i.style.margin="0",i.style.background="#ffffff";const s=document.createElement("style");s.textContent=z(a,"#pdf-scope-materials"),i.appendChild(s);const p=document.createElement("div");p.innerHTML=d,i.appendChild(p),o.appendChild(i),document.body.appendChild(o),await new Promise(f=>{const m=new Image;m.onload=()=>f(),m.onerror=()=>f(),m.src=`${window.location.origin}/logo-print.png`,setTimeout(f,3e3)});try{const f=(await $(async()=>{const{default:m}=await import("./html2pdf-BIYreVBH.js").then(c=>c.h);return{default:m}},__vite__mapDeps([0,1,2]))).default;await f().set({margin:[12,12,12,12],filename:`Смета на материал ${t.object_code}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff",width:760,windowWidth:760,scrollX:0,scrollY:0},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["css","legacy"],avoid:[".cat-block",".room-block"]}}).from(i).save()}finally{o.parentNode&&document.body.removeChild(o)}}export{j as d,E as p};
