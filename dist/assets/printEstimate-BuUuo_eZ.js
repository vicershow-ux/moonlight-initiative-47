...
import{a9 as E}from"./index-rP7_InGR.js";const b=(t,o=0)=>{const d=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(d)?d:o},f=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(b(t))+" ₽",y=t=>new Date(t).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"}),z=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function $(t,o,d){const u=t.items||[],s=new Map;u.forEach(h=>{const g=h.room_name||"Без помещения";s.has(g)||s.set(g,[]),s.get(g).push(h)});const e=b(t.subtotal_amount??t.total_amount),i=b(t.discount_amount??0),n=Array.from(s.entries()).map(([h,g])=>{const v=new Map;g.forEach(x=>{const l=x.category||"Прочие работы";v.has(l)||v.set(l,[]),v.get(l).push(x)});const k=g.reduce((x,l)=>x+b(l.amount),0),_=Array.from(v.entries()).map(([x,l])=>{const A=l.reduce((c,w)=>c+b(w.amount),0),C=l.map((c,w)=>`
            <tr>
              <td class="num">${w+1}</td>
              <td>${a(c.name)}</td>
              <td class="center">${a(c.unit)}</td>
              <td class="center">${b(c.quantity)}</td>
              <td class="center">${b(c.times,1)}</td>
...
`,p=`
  ...
  </div>`,m=`Смета № ${t.id} — ${a(o.client_name)}`;return{styles:r,bodyContent:p,title:m}}function T(t,o,d){const{styles:u,bodyContent:s,title:e}=$(t,o,d),i=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${e}</title>
...
</html>`,n=window.open("","_blank","width=900,height=1000");n&&(n.document.open(),n.document.write(i),n.document.close(),n.onload=()=>{n.focus(),n.print()})}function D(t,o){return t.replace(/@page[^{]*\{[^}]*\}/g,"").replace(/@media\s+print\s*\{(?:[^{}]*\{[^}]*\})*[^{}]*\}/g,"").replace(/(^|\})\s*([^{}@]+)\s*\{/g,(u,s,e)=>{const i=e.split(",").map(n=>{const r=n.trim();return r?r==="*"?`${o}, ${o} *`:/^(html|body)$/i.test(r)||r===".est-root"?o:r.startsWith(".est-root ")?`${o} ${r.slice(10)}`:`${o} ${r}`:""}).filter(Boolean).join(", ");return`${s} ${i} {`})}async function F(t,o,d){const{styles:u,bodyContent:s}=$(t,o,d),e=document.createElement("div");e.style.position="fixed",e.style.left="-10000px",e.style.top="0",e.style.width="760px",e.style.background="#ffffff";const i=document.createElement("div");i.id="pdf-scope-estimate",i.className="est-root",i.style.width="760px",i.style.maxWidth="760px",i.style.padding="0",i.style.margin="0",i.style.background="#ffffff";const n=document.createElement("style");n.textContent=D(u,"#pdf-scope-estimate"),i.appendChild(n);const r=document.createElement("div");r.innerHTML=s,i.appendChild(r),e.appendChild(i),document.body.appendChild(e),await new Promise(p=>{const m=new Image;m.onload=()=>p(),m.onerror=()=>p(),m.src=`${window.location.origin}/logo-print.png`,setTimeout(p,3e3)});try{const p=(await E(async()=>{const{default:m}=await import("./html2pdf-CzN2CxGO.js").then(h=>h.h);return{default:m}},__vite__mapDeps([0,1,2]))).default;await p().set({margin:[12,12,12,12],filename:`Смета №${t.id}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff",width:760,windowWidth:760,scrollX:0,scrollY:0},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["css","legacy"],avoid:[".cat-block",".room-block"]}}).from(i).save()}finally{e.parentNode&&document.body.removeChild(e)}}function a(t){const o=document.createElement("div");return o.textContent=t??"",o.innerHTML}export{F as d,T as p};
