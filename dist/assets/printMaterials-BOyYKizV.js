const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-CzN2CxGO.js","assets/index-rP7_InGR.js","assets/index-lj-BAnLg.css"])))=>i.map(i=>d[i]);
import{a9 as y}from"./index-rP7_InGR.js";const b=(t,a=0)=>{const n=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(n)?n:a},u=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(b(t))+" ₽",k=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function r(t){const a=document.createElement("div");return a.textContent=t??"",a.innerHTML}function $(t,a){const n=new Map;return t.forEach(d=>{const e=a.find(i=>i.id===d.material_id),l=d.shop_name||(e==null?void 0:e.shop_name)||"Магазин не указан";n.has(l)||n.set(l,{name:l,address:"",phone:"",items:[],sum:0});const o=n.get(l);!o.address&&(e!=null&&e.shop_address)&&(o.address=e.shop_address),!o.phone&&(e!=null&&e.shop_phone)&&(o.phone=e.shop_phone),o.items.push(d),o.sum+=b(d.qty)*b(d.price)}),Array.from(n.values()).sort((d,e)=>e.sum-d.sum)}function v(t,a,n,d){const e=$(a,n),l=e.reduce((c,g)=>c+g.sum,0),o=Array.from(new Set(a.map(c=>c.room_name).filter(Boolean))),i=o.length?o.join(", "):"Без помещения",s=e.map(c=>{const g=c.items.map((m,w)=>`
            <tr>
              <td class="num">${w+1}</td>
              <td>${r(m.name)}</td>
              <td>${r(m.room_name||"—")}</td>
              <td>${r(m.work_type||"—")}</td>
              <td class="center">${b(m.qty)}</td>
              <td class="center">${r(m.unit)}</td>
              <td class="right">${u(m.price)}</td>
              <td class="right amount">${u(b(m.qty)*b(m.price))}</td>
            </tr>`).join(""),x=c.address?`Адрес: ${r(c.address)}`:"";return`
      <div class="room-block">
        <div class="cat-block">
          <table>
            ...
            <tbody>
              <tr class="cat-row"><td colspan="8">${r(c.name)}${x?` — ${x}`:""}</td></tr>
              ${g}
            </tbody>
            <tfoot>
              <tr>
...
`,f=`
  ...
  <div class="info-grid">
    <div>
      ...
      <div class="value">${r(t.object_code)}</div>
    </div>
    <div>
      <div class="label">Заказчик</div>
      <div class="value">${r(t.client_name)}</div>
    </div>
    <div>
      <div class="label">Адрес объекта</div>
      <div class="value">${r(t.address||"—")}</div>
    </div>
    <div>
      <div class="label">Помещение</div>
      <div class="value">${r(i)}</div>
    </div>
  </div>
  ...
  <div class="parties">
    ...
    <div>
      <div class="label">Заказчик</div>
      <div class="name">${r(t.client_name)}</div>
    </div>
  </div>

  <div class="footer">
    Сформировано ${k(new Date().toISOString())}
  </div>`,h=`Смета на материал № ${t.object_code} — ${r(t.client_name)}`;return{styles:p,bodyContent:f,title:h}}function C(t,a,n,d,e=!1){const{styles:l,bodyContent:o,title:i}=v(t,a,n,d),s=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${i}</title>
...
</html>`,p=window.open("","_blank","width=900,height=1000");p&&(p.document.open(),p.document.write(s),p.document.close(),e&&(p.onload=()=>{p.focus(),p.print()}))}function A(t,a){return t.replace(/@page[^{]*\{[^}]*\}/g,"").replace(/@media\s+print\s*\{(?:[^{}]*\{[^}]*\})*[^{}]*\}/g,"").replace(/(^|\})\s*([^{}@]+)\s*\{/g,(d,e,l)=>{const o=l.split(",").map(i=>{const s=i.trim();return s?s==="*"?`${a}, ${a} *`:/^(html|body)$/i.test(s)||s===".est-root"?a:s.startsWith(".est-root ")?`${a} ${s.slice(10)}`:`${a} ${s}`:""}).filter(Boolean).join(", ");return`${e} ${o} {`})}async function E(t,a,n,d){const{styles:e,bodyContent:l}=v(t,a,n,d),o=document.createElement("div");o.style.position="fixed",o.style.left="-10000px",o.style.top="0",o.style.width="760px",o.style.background="#ffffff";const i=document.createElement("div");i.id="pdf-scope-materials",i.className="est-root",i.style.width="760px",i.style.maxWidth="760px",i.style.padding="0",i.style.margin="0",i.style.background="#ffffff";const s=document.createElement("style");s.textContent=A(e,"#pdf-scope-materials"),i.appendChild(s);const p=document.createElement("div");p.innerHTML=l,i.appendChild(p),o.appendChild(i),document.body.appendChild(o),await new Promise(f=>{const h=new Image;h.onload=()=>f(),h.onerror=()=>f(),h.src=`${window.location.origin}/logo-print.png`,setTimeout(f,3e3)});try{const f=(await y(async()=>{const{default:h}=await import("./html2pdf-CzN2CxGO.js").then(c=>c.h);return{default:h}},__vite__mapDeps([0,1,2]))).default;await f().set({margin:[12,12,12,12],filename:`Смета на материал ${t.object_code}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff",width:760,windowWidth:760,scrollX:0,scrollY:0},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["css","legacy"],avoid:[".cat-block",".room-block"]}}).from(i).save()}finally{o.parentNode&&document.body.removeChild(o)}}export{E as d,C as p};
