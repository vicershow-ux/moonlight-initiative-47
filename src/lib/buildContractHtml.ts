...
export function buildContractHtml(ctx: ContractContext): string {
  ...
  const materials41 = {
    customer: "Закупка строительных и отделочных материалов, необходимых для выполнения Работ, осуществляется за счёт Заказчика.",
    contractor: "Закупка строительных и отделочных материалов, необходимых для выполнения Работ, осуществляется за счёт Подрядчика. Стоимость материалов включена в цену Работ.",
    mixed: "Закупка материалов осуществляется Сторонами совместно: черновые и расходные материалы приобретает Подрядчик, чистовые и отделочные материалы — Заказчик. Конкретный перечень определяется Сметой (Приложение №1).",
  }[o.materials || "customer"]

  const materialsHtml = `<h3>4. Закупка материалов</h3>
<p>4.1. ${materials41}</p>
...
<p>4.6. Подрядчик не несёт ответственности за качество строительных и отделочных материалов, приобретённых Заказчиком самостоятельно, и вправе отказаться от их использования, если они имеют явные дефекты или не соответствуют технологическим требованиям.</p>`
  ...
  const docDelivery53 = {
    personal_messengers: "лично под подпись либо через мессенджеры по контактным данным, указанным в реквизитах Сторон.",
    personal_mail: "лично под подпись либо заказным письмом с уведомлением по адресам, указанным в реквизитах Сторон.",
    email: "по электронной почте по адресам, указанным в реквизитах Сторон. Документы, направленные по электронной почте, признаются Сторонами имеющими юридическую силу.",
...
<p>5.3. Направление Актов, уведомлений и иных документов производится ${docDelivery53}</p>`
