const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-C_VEGgiI.js","assets/index-B2HRQ85N.js","assets/index-C7rVwvzy.css"])))=>i.map(i=>d[i]);
import{_ as E}from"./index-B2HRQ85N.js";const u=(t,o=0)=>{const d=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(d)?d:o},g=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(u(t))+" ₽",w=t=>new Date(t).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"}),z=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function $(t,o,d){const h=t.items||[],s=new Map;h.forEach(f=>{const x=f.room_name||"Без помещения";s.has(x)||s.set(x,[]),s.get(x).push(f)});const n=u(t.subtotal_amount??t.total_amount),i=u(t.discount_amount??0),e=Array.from(s.entries()).map(([f,x])=>{const v=new Map;x.forEach(b=>{const l=b.category||"Прочие работы";v.has(l)||v.set(l,[]),v.get(l).push(b)});const _=x.reduce((b,l)=>b+u(l.amount),0),k=Array.from(v.entries()).map(([b,l])=>{const A=l.reduce((c,y)=>c+u(y.amount),0),C=l.map((c,y)=>`
            <tr>
              <td class="num">${y+1}</td>
              <td>${a(c.name)}</td>
...
`,p=`
  ...
  </div>`,m=`Смета № ${t.id} — ${a(o.client_name)}`;return{styles:r,bodyContent:p,title:m}}function j(t,o,d){const{styles:h,bodyContent:s,title:n}=$(t,o,d),i=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${n}</title>
