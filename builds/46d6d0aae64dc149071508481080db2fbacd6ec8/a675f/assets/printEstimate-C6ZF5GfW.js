const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-Cbf6wlcD.js","assets/index-C5Ch91G9.js","assets/index-C7rVwvzy.css"])))=>i.map(i=>d[i]);
import{_ as C}from"./index-C5Ch91G9.js";const g=(t,a=0)=>{const e=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(e)?e:a},m=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(g(t))+" ₽",w=t=>new Date(t).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"}),E=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function $(t,a,e){const h=t.items||[],r=new Map;h.forEach(x=>{const u=x.room_name||"Без помещения";r.has(u)||r.set(u,[]),r.get(u).push(x)});const n=g(t.subtotal_amount??t.total_amount),l=g(t.discount_amount??0),i=Array.from(r.entries()).map(([x,u])=>{const v=new Map;u.forEach(f=>{const d=f.category||"Прочие работы";v.has(d)||v.set(d,[]),v.get(d).push(f)});const _=u.reduce((f,d)=>f+g(d.amount),0),k=Array.from(v.entries()).map(([f,d])=>{const z=d.reduce((s,y)=>s+g(y.amount),0),A=d.map((s,y)=>`
            <tr>
              <td class="num">${y+1}</td>
              <td>${o(s.name)}</td>
              <td class="center">${o(s.unit)}</td>
              <td class="center">${g(s.quantity)}</td>
              <td class="center">${g(s.times,1)}</td>
              <td class="right">${m(s.price)}</td>
              <td class="right amount">${m(s.amount)}</td>
            </tr>`).join("");return`
        <div class="cat-block">
          <table>
            <thead>
...
`,c=`
  ...
  <div class="doc-subtitle">
    ...
    ${t.contract_number?`<p>Приложение к договору № ${o(t.contract_number)}${t.contract_date?` от ${w(t.contract_date)}`:""}</p>`:""}
  </div>
  ...
  </div>`,p=`Смета № ${t.id} — ${o(a.client_name)}`;return{styles:b,bodyContent:c,title:p}}function D(t,a,e){const{styles:h,bodyContent:r,title:n}=$(t,a,e),l=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${n}</title>
...
<body>
<div class="est-root">${r}</div>
</body>
</html>`,i=window.open("","_blank","width=900,height=1000");i&&(i.document.open(),i.document.write(l),i.document.close(),i.onload=()=>{i.focus(),i.print()})}async function T(t,a,e){const{styles:h,bodyContent:r}=$(t,a,e),n=document.createElement("div");n.style.position="fixed",n.style.left="-10000px",n.style.top="0",n.style.width="850px",n.style.background="#ffffff";const l=document.createElement("style");l.textContent=h;const i=document.createElement("div");i.className="est-root",i.style.width="850px",i.style.margin="0",i.innerHTML=r,n.appendChild(l),n.appendChild(i),document.body.appendChild(n),await new Promise(c=>{const p=new Image;p.onload=()=>c(),p.onerror=()=>c(),p.src=`${window.location.origin}/favicon.png`,setTimeout(c,3e3)});const b=(await C(async()=>{const{default:c}=await import("./html2pdf-Cbf6wlcD.js").then(p=>p.h);return{default:c}},__vite__mapDeps([0,1,2]))).default;await b().set({margin:[10,10,10,10],filename:`Смета №${t.id}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff"},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"}}).from(i).save(),document.body.removeChild(n)}function o(t){const a=document.createElement("div");return a.textContent=t??"",a.innerHTML}export{T as d,D as p};
