const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-6mj_tdAY.js","assets/index-CD2-_ERK.js","assets/index-C7rVwvzy.css"])))=>i.map(i=>d[i]);
import{_ as E}from"./index-CD2-_ERK.js";const u=(t,o=0)=>{const d=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(d)?d:o},h=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(u(t))+" ₽",y=t=>new Date(t).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"}),z=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function $(t,o,d){const g=t.items||[],s=new Map;g.forEach(f=>{const x=f.room_name||"Без помещения";s.has(x)||s.set(x,[]),s.get(x).push(f)});const n=u(t.subtotal_amount??t.total_amount),i=u(t.discount_amount??0),e=Array.from(s.entries()).map(([f,x])=>{const v=new Map;x.forEach(b=>{const l=b.category||"Прочие работы";v.has(l)||v.set(l,[]),v.get(l).push(b)});const _=x.reduce((b,l)=>b+u(l.amount),0),k=Array.from(v.entries()).map(([b,l])=>{const A=l.reduce((c,w)=>c+u(w.amount),0),C=l.map((c,w)=>`
            <tr>
              <td class="num">${w+1}</td>
              <td>${a(c.name)}</td>
              <td class="center">${a(c.unit)}</td>
              <td class="center">${u(c.quantity)}</td>
              <td class="center">${u(c.times,1)}</td>
              <td class="right">${h(c.price)}</td>
              <td class="right amount">${h(c.amount)}</td>
            </tr>`).join("");return`
        <div class="cat-block">
          <table>
            <thead>
            ...
            <tfoot>
              <tr>
                ...
                <td class="right amount">${h(A)}</td>
              </tr>
            </tfoot>
          </table>
        </div>`}).join("");return`
      <div class="room-block">
        <h3 class="room-title">${a(f)}</h3>
        ${k}
        <div class="room-total">
          <div>
            <div class="room-total-label">Итоговая сумма по помещению</div>
            <div class="room-total-name">${a(f)}</div>
          </div>
          <div class="room-total-amount">${h(_)}</div>
        </div>
      </div>`}).join(""),r=`
...
`,p=`
  ...
  </div>`,m=`Смета № ${t.id} — ${a(o.client_name)}`;return{styles:r,bodyContent:p,title:m}}function j(t,o,d){const{styles:g,bodyContent:s,title:n}=$(t,o,d),i=`<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8" />
<title>${n}</title>
