const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/html2pdf-Dk0NfHa-.js","assets/index-D2T0nmou.js","assets/index-C7rVwvzy.css"])))=>i.map(i=>d[i]);
import{_ as A}from"./index-D2T0nmou.js";const f=(t,o=0)=>{const d=typeof t=="string"?parseFloat(t):Number(t);return Number.isFinite(d)?d:o},u=t=>new Intl.NumberFormat("ru-RU",{maximumFractionDigits:0}).format(f(t))+" ₽",w=t=>new Date(t).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"}),E=t=>new Date(t).toLocaleString("ru-RU",{day:"numeric",month:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});function $(t,o,d){const h=t.items||[],s=new Map;h.forEach(g=>{const b=g.room_name||"Без помещения";s.has(b)||s.set(b,[]),s.get(b).push(g)});const n=f(t.subtotal_amount??t.total_amount),i=f(t.discount_amount??0),e=Array.from(s.entries()).map(([g,b])=>{const v=new Map;b.forEach(x=>{const l=x.category||"Прочие работы";v.has(l)||v.set(l,[]),v.get(l).push(x)});const _=b.reduce((x,l)=>x+f(l.amount),0),k=Array.from(v.entries()).map(([x,l])=>{const z=l.reduce((c,y)=>c+f(y.amount),0),C=l.map((c,y)=>`
            <tr>
              <td class="num">${y+1}</td>
              <td>${a(c.name)}</td>
